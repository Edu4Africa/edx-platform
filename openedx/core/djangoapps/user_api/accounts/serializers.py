...
class AccountFullUserProfileReadOnlySerializer(serializers.Serializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Don't pass the 'use_filter' arg up to the superclass
        self.use_filter = kwargs.pop('use_filter', True)
    ...
    def to_native(self, user):
        ...
        data = {
            ...
            "url": ''.join(['http://', get_current_site(None).domain, reverse('accounts_api', kwargs={'username': user.username})]),
            "email": user.email,
            ...
            "time_zone": None,
            "language_proficiencies": LanguageProficiencySerializer(profile.language_proficiencies.all(), many=True).data,
            "name": profile.name,
